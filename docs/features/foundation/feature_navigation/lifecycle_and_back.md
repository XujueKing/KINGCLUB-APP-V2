...
| 当前页面 | 系统/手势返回 | 成功出口 |
|---|---|---|
| `/auth/bootstrap` | 禁止 pop | reset mobile、home 或安全 pending 目标 |
| `/auth/mobile` | 无公开来源时二次确认退出 App；有批准公开来源时返回该来源 | replace code |
...
| `/home` 根页 | 按平台根页规范退出/后台，不回 auth flow | future protected 路由 push |
...
- 推送点击、深链、K104 完成和撤销事件同时到达时，安全会话 reset 优先于业务目标；被撤销后不得消费 pending protected intent。

## 4. V1 状态恢复决策

- 首个移动端版本不为 `GoRouter/MaterialApp.router` 启用路由树 restoration，冷启动固定从 `/auth/bootstrap` 重建。
- auth flow、`$extra`、FlowStore、pending intent 和通用 notice 永不持久化。
...
- anonymous 安全 fallback 为 mobile；authenticated 为 home；restricted 为已批准 restricted 目标，未实现时清除敏感流程后回 mobile。
