...
- 设计版本：`Legacy Four-Tab Shell Replica v2`
- 批准日期：2026-08-26
- UI 状态：`UI Mock Implemented`（2026-08-28，离线 Mock 已正式验收）

## 页面目标

承载小程序原版四个稳定主目的地、旧版悬浮胶囊视觉、安全区域、分支内容、全局网络提示和消息徽标。业务页面决定自己的内容，Shell 不展示服务端动态菜单。
...
- 四个目的地使用旧版真实图标、顺序、圆形选中底和悬浮胶囊；无视觉文字时必须提供完整无障碍标签。
- 2026-09-12 用户以“我的”页图一/图二对照明确要求恢复小程序原版四项底栏；内容页保留独立路由，不再占用底部主目的地。
...
| Primary Navigation | 首页、消息、私人储物柜、我的 |
...
详见 [interactions.md](interactions.md)。四分支进程内保活、重按 Tab 回根/滚动到顶；扫码关闭后恢复首页来源状态。
...
- Android API 37、1080×2400 需依次检查首页、消息、私人储物柜和我的四个稳定主分支。
