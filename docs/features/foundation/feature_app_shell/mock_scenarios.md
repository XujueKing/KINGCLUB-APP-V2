...
| 场景 ID | 输入 | 预期 |
|---|---|---|
| SHELL-M01 | approved 会员首次登录 | 首页根启动，五个固定主目的地可见；扫码只存在于首页批准入口 |
| SHELL-M02 | 切换五个 Tab 后返回 | 各分支栈和滚动状态在进程内保留 |
| SHELL-M03 | 当前消息分支在单聊详情，重按消息 | 回会话列表根；再次点击发出滚动到顶意图 |
| SHELL-M04 | 从批准入口打开扫码后取消 | 返回原分支和原位置，扫码不成为第六分支 |
| SHELL-M05 | 系统通知 3 + 好友会话 2；依次阅读一条系统、全部系统、好友会话；另测聚合 0/1/99/120 | 徽标依次为 5、4、2、隐藏；边界为隐藏、1、99、99+；仅切 Tab 不清零，语义朗读正确 |
| SHELL-M06 | 离线/恢复在线 | 全局提示出现/消失，当前分支不被重建 |
| SHELL-M07 | 前台收到 session revoked | 等待清理，销毁五分支后 reset 手机号登录 |
