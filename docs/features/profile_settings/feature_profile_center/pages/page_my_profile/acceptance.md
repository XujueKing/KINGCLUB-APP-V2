# 我的主页验收

## 2026-09-12 资产型小程序页面

- [x] 用户提供截图并明确要求替换现有“我的”页面及其全部内容。
- [x] 已定位对应旧版 `5406b87 / 1.1.31` 的 WXML/WXSS 结构与资源。
- [x] 总余额、钱包账户、代金券账户、金币 200、钻石 0 与四项菜单按截图完成。
- [x] 设置、二维码、个人信息、账单和关于入口均可进入并正常返回。
- [x] 现有 App Shell 底栏保持单一来源且“我的”选中。
- [x] Android 真机截图与用户视觉真值完成同状态对比。
- [x] 定向测试与 `flutter analyze` 通过。

以下 `Legacy My Profile Replica v1` 验收为历史版本记录，不代表当前页面目标。

## 文档准入

- [x] 用户于 2026-08-26 提供旧版完整首屏截图作为视觉真值。
- [x] 用户确认头像内部保持空白，其余可见内容和入口全部复刻。
- [x] 页面仍处于离线 UI/Fake 阶段，不连接真实资料、资产、社交或二维码服务。
- [x] 旧版完整内容的隐私边界已记录，真实接入前必须重新评审。

## Legacy My Profile Replica v1

- [x] 上海夜景封面、深棕黑面板及旧版信息层级完成。
- [x] 空白头像、昵称、等级、Fake 账号与复制入口完成。
- [x] 获赞、关注、互关、粉丝及编辑主页入口完成。
- [x] 余额、金币、钻石及旧版标签完成。
- [x] 作品、动态、相册三项可切换，默认动态且内容区保持截图空白状态。
- [x] 小下箭头跟随当前选中项；作品、动态、相册选中时均在对应文字右侧显示，且全栏始终只有一个箭头。
- [x] 二维码、设置、EXP、统计、资产、账号复制和编辑主页均有可返回 Fake 交互。
- [x] 底部“我的”选中，微信宿主胶囊未带入 App。
- [x] Android API 37 / 1080×2400 模拟器实机截图通过。
- [x] `flutter analyze` 无问题，全部 10 项测试通过。
- [x] 未访问真实接口、WebSocket、支付、推送、系统剪贴板或生产 SDK。

## 2026-08-28 左上工具栏复验

- [x] 二维码和设置图标显示尺寸一致，均按旧版 `40rpx` 换算。
- [x] 两图标中心间距为显示宽度的 2 倍，点击热区互不重叠且两个入口仍可正常返回。
- [ ] Android 新裁片经用户确认后再恢复页面视觉通过状态。

## 2026-08-28 整页字体复验

- [x] 昵称、账号、等级、统计、资产、资料标签和内容页签按旧版 `750rpx` 字号换算。
- [x] 取消不属于旧版的 `w700/w800` 粗重效果，文字基线及块间距与旧版截图一致。
- [ ] Android 新截图经用户确认后再恢复页面视觉通过状态。

## 2026-08-28 头图信息排版复验

- [x] 头像按旧版 `180rpx` 尺寸，只压住 `540rpx` 主面板分界约 `40rpx`。
- [x] 昵称、等级和账号与头像垂直居中，保持旧版左右间距及上下关系。
- [x] 四项统计不再均分拉伸；编辑按钮和资产行按旧版位置对齐。
- [ ] Android 新截图经用户确认后再恢复页面视觉通过状态。

验收证据：[android_my_profile_header_layout_v2.png](screenshots/android_my_profile_header_layout_v2.png)。

## 2026-08-29 我的订单入口

- [x] 资产服务区域新增“我的订单”胶囊按钮，不覆盖旧版资料内容
- [x] 点击只触发订单中心路由，不传订单、金额或账号对象
- [x] 订单中心返回后仍停留在“我的”Tab
- [x] 缺少外部路由回调时提供正式降级反馈，不显示 Fake/Mock 说明
- [x] Android 1080 × 2400 真机完成“我的 → 我的订单 → 详情 → 返回”验收
- [x] “我的订单”使用香槟金实底突出，且胶囊高度与余额、金币、钻石完全一致
- [x] 四个资产入口使用同一固定高度约束，而不是各自内容高度或纵向 Padding 推导

验收证据：

- [我的页面订单入口](audit/2026-08-29-orders-entry/01-my-orders-entry.png)
- [订单中心](audit/2026-08-29-orders-entry/02-order-center.png)
- [AA 订单详情](audit/2026-08-29-orders-entry/03-aa-order-detail.png)
- [返回“我的”保持 Tab](audit/2026-08-29-orders-entry/04-back-to-my.png)

## 2026-08-29 封面编辑联动

- [x] 编辑主页保存新封面后，“我的”页面立即更新且资料文字叠放关系不变
- [x] 放弃编辑或取消封面选择时主页封面不变
- [x] 封面联动仅使用本地 Mock 资源，不访问相册、上传或真实资料接口

验收证据：[`../page_edit_profile/audit/2026-08-29-black-gold-cover/README.md`](../page_edit_profile/audit/2026-08-29-black-gold-cover/README.md)
