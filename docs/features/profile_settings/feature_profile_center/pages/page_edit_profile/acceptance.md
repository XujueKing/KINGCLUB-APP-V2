...
## 2026-09-13 资料完整性
保留旧版会员称呼、签名、年龄性别、颜值、能量等级、灵根、手机、会员号、支付密码入口；新增体重、城市选择和注册选项多选。身高 100–250cm 整数刻度，体重 30–250kg 每格0.5；保存到 details.weightKg 和 details.interests（结构化选项ID）。城市离线数据来自 https://github.com/modood/Administrative-divisions-of-China （2023数据，WTFPL）；定位独立授权，不影响手动选择。

验证：静态检查通过，22 项资料/路由/刻度/城市测试通过。APK 构建并覆盖安装成功。个人手机号仅显示已有会话脱敏号码；历史会话未保留号码时显示已绑定，不填旧库或测试号码。支付密码入口沿用现有页面，本次不宣称支付后端已接通。

## 2026-09-13 城市定位反馈与兼容性
定位中在城市行显示获取位置/识别城市，防止重复请求；Android 使用系统 LocationManager，优先使用三分钟内且精度不差于 3km 的近期位置，避免不必要等待。新位置请求和城市反查均设超时。权限/定位服务未开启时提供设置入口，失败采用当前屏幕可见的提示，保留原城市与手动选择。成功识别后仍需保存资料。日志只记录失败阶段和异常类型，不记录坐标。

验证：flutter analyze 无问题，14 项资料编辑及城市选择测试通过，Android preview profile APK 构建成功。第一版已覆盖安装，真机城市行已显示 Hunan · Zhuzhou。继续修正 geocoding_android 5.1.0 将 Locale.toString() 传给 Locale.forLanguageTag 的格式不兼容问题：中文请求使用 zh，避免 zh_CN 被解析为空语言。中文修正版覆盖安装成功；真机主页已显示“湖南省 · 株洲市”，确认中文城市结果已回显。
