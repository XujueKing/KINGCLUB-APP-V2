# 一起玩 AA 预订

- Scope ID：`KC-F-023`
- 文档状态：`Approved for Development`
- 所属业务域：`club`
- M0 范围：`In Release Scope`
- 设计版本：`AA Reservation Legacy Replica v2`
- 最后更新：2026-08-31

## 目标与用户价值

让已通过会员审核的成年人按服务端开放日期查看 AA 场次和套餐，获得可解释的权威报价，在库存短时占位内确认订单，再进入统一支付流程。

## 已确认事实

- 旧版主链为 `Choose → order → order2 → pay`，另外从已支付预订进入 `ticket`。
- 旧客户端按年龄直接减 180/120/60 元，并在本地组合优惠券、金币、余额、赠券、经验和信用分。
- 旧版把订单 JSON、`userAccount` 和 `gender` 放入路由，把客户端金额作为支付请求字段。
- 旧页面固定显示 20:30/21:00～04:00，同时数据库过程又以 04:00 切业务日，存在文案和权威时间不一致。
- 本期已纳入统一订单中心 KC-P-036、订单详情 KC-P-037、支付 KC-P-038 和入场凭证 KC-P-033，但这些页面仍待单独设计。
- 用户于 2026-08-31 确认：“一起玩”不允许会员在预订时选择或得知卡座号；预订只锁定本人一席，具体卡座由系统随机分配。
- 用户于 2026-08-31 确认：只在该场预订营业日的前一天揭晓本人卡座号，机制类似演唱会座位揭晓。

## 当前建议

- 预订资格：登录有效、会员审核通过、成年且账号可用；具体资格由服务端返回，不由客户端从生日、性别或本地资料推导。
- 每位会员每个 `serviceDate` 最多拥有一个 `pendingPayment | confirmed` 的 AA 席位；仅为本人预订一席，服务端并发约束兜底。
- 营业时间、销售截止时间、库存、套餐、限制、优惠和退款规则全部来自版本化服务端投影。
- 取消旧版“颜值分”展示和自动筛选；普通可订套餐不展示同桌会员资料。用户于 2026-08-29 明确要求：已有已支付选座的定位卡必须复刻旧版男女席位占位矩阵，此矩阵只表达座位占用状态，不展示头像、昵称或真实身份。
- 取消客户端按性别赠券和硬编码年龄减价。若保留活动优惠，必须由服务端活动规则返回资格、说明和价格明细。
- 入口使用“随机预订”或“系统推荐套餐”，不再使用“选座”文案；仍必须经过套餐详情和订单确认，不得一键下单。
- 套餐、报价、待支付和普通已确认投影不得包含可见卡座号。卡座号只存在于进入揭晓窗口后的分配投影和入场凭证中。
- 列表与详情不锁库存；用户在确认提交时由服务端原子创建短时席位占位和待支付订单。
- KC-P-029 只确认报价和抵扣；真正拉起支付、处理中、成功/失败复核归 KC-P-038。

## 旧版外观复刻边界

- 本批次以旧版 `Choose → order → order2` 的黑金/粉金视觉、信息层级、日期条、套餐卡、固定底栏和返回路径为直接基准，不再改成新的通用商城卡片。
- “完全复刻”指 App 自有页面的视觉结构和操作节奏；微信小程序右上角胶囊、微信支付 SDK、系统状态栏等平台部件不复制。
- 旧版客户端年龄减价、性别赠券、颜值筛选、同桌头像/性别占位和客户端金额计算属于已审计风险，不进入 V2 Fake 数据与后续真实契约。
- UI 阶段全部使用本地 Fake 套餐、Fake 报价、Fake 占位倒计时和 Fake 支付出口；任何按钮都不调用超级接口、WebSocket 或支付 SDK。

## 页面与文档

- [KC-P-027 一起玩 AA 预订页](pages/page_aa_reservations/README.md) — `UI Mock Implemented`
- [KC-P-028 AA 卡座套餐详情页](pages/page_aa_package_detail/README.md) — `UI Mock Implemented`
- [KC-P-029 AA 确认订单页](pages/page_aa_order_confirmation/README.md) — `UI Mock Implemented`
- [AA 已选座定位凭证页](pages/page_aa_positioning_card/README.md) — `Approved for Development`
- [旧版审计](legacy_audit.md)
- [流程与导航](flow_and_navigation.md)
- [状态机](state_machine.md)
- [数据与 Fake 契约](data_and_api.md)
- [价格、库存与安全](pricing_inventory_security.md)
- [Mock 场景](mock_scenarios.md)
- [功能验收](acceptance.md)

## 本期不包含

- 多人代订、自由选桌、组长建局、拼桌群聊、候补、转让、退款自助处理。
- 在 AA 页面直接完成支付；已支付选座可按用户确认的旧版流程展示定位卡和本地动态入场二维码 Mock。
- 真实超级接口、支付 SDK、推送或实时库存接入。

## 已确认决策

1. 同意每人每个营业日只允许一条待支付或已确认 AA 预订，且只订本人一席。
2. 同意移除消费者界面的“颜值分”、同桌头像/性别占位和按性别赠券；活动优惠统一由服务端规则返回。
3. 同意“系统推荐套餐”也必须经过详情和确认，不提供真正的一键下单。
4. 同意 KC-P-029 只创建短时占位和待支付订单，支付过程与最终结果统一交给 KC-P-038。
5. 同意已支付后的取消/退款本期不在 AA 页面自助处理，后续在订单模块确定客服或退款流程。
6. 用户于 2026-08-29 追加确认：有已支付选座时，列表展示旧版紫色定位卡；点击整卡进入旧版 `POSITIONING CARD` 二维码页。
7. 2026-08-31 更正决策 6 的时机：已支付不等于已揭晓卡座。揭晓窗口前只显示“预订已确认·卡座待揭晓”；营业日前一天才展示紫色定位卡和具体卡座号。

## 开发门禁

本功能已于 2026-08-28 完成 UI/Mock 主流程、异常矩阵与 Android 视觉验收；全局 `UI Flow Approved` 前仍不接真实预订、库存、资产或支付服务。
