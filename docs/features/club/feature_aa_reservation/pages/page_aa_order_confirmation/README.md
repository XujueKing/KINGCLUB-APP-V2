...
```text
[旧版返回箭头]          确认订单
[黑金分区：分配方式 / 时间 / 套餐 / AA人次价]
[黑金分区：优惠券 / 金币方案 / 余额方案]
[黑金分区：微信支付]
[规则确认与报价倒计时]

[米金固定底栏：实付金额]                 [深棕胶囊立即付款]
```

- 复刻旧版 `order2` 的分区行和固定付款底栏，综合色彩跟随详情页的黑金、米金与深棕体系；禁止粉色、玫红和酒红背景。
- 抵扣选项触发本地 Fake 重新报价；报价变化后清除规则勾选。
- “立即付款”只生成 Fake 待支付结果并显示支付模块未接入说明，不拉起微信支付。
- 确认页固定显示“分配方式：系统随机分配”和“营业日前一天揭晓卡座”，禁止显示 V5/A6/888 等编号。
...
- 规则勾选框、`我已阅读并同意本次预订、迟到和取消规则` 与 `查看` 必须在同一行垂直居中。
- `查看` 紧跟规则文字，仅保留正常字间距，不使用 `Expanded` 把按钮推到屏幕最右侧，也不得形成明显空白断层。
- 整行作为一个紧凑内容组左对齐展示；规则勾选框必须与上方优惠券、金币兑换、余额三个勾选框处在同一条竖线上。规则文字保持单行，不拆成错位的上下两行。

## AA 支付页交接

- 正常报价勾选规则并提交后，不再停留在“订单已提交”对话框；使用替换式导航进入已批准的支付处理与结果页。
- 页面只传递本地 Fake `PaymentIntentRef`，不得把金额、用户信息或支付成功标记作为路由参数传递。
- 支付页根据 Fake 引用读取 AA 固定夹具，展示“随机卡座·待揭晓”、`3880卡座套餐` 和确认页最新应付金额；返回时不得再次提交原报价。
- 本阶段仅连接本地 Fake 页面流程，不调用超级接口、支付 SDK 或真实订单服务。
