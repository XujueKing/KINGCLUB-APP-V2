...
- “一键随机选座”在 Fake 阶段选择第一个可用套餐并进入详情，不跳过详情或确认页。

- 日期使用服务端 `ServiceDay`，不在 UI 硬编码 14 天或营业时段。
- 已有有效预订优先展示；同一营业日有待支付/已确认预订时禁用新订入口。`08.27` 本地 Mock 为已支付选座状态，使用旧版紫色定位卡完整展示 `888`、场次、席位矩阵、套餐、单人价格和二维码入口。
...
- `readyWithConfirmed`：顶部使用用户提供的旧版紫色定位卡，不显示通用“已确认”提示卡；整卡可点击进入 `POSITIONING CARD` 二维码页，同日所有新订入口禁用。
