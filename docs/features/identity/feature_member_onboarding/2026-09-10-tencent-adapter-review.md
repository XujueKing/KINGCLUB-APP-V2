...
**2026-09-10 晚间补录**：短信登录已接入并部署，旧短信正文恢复已完成；当前测试入口为 `https://test.wuyexin.cn/kingclub-v2`，目标 `api.sh-kingclub.cn` 备案处理中。下方“真实接入未开始”仅适用于照片实名/评分链，不适用于已部署短信。下一步补真实会话/会员状态与申请衔接，正式照片 adapter 仍待开发。依据见[接续记录](../../../migration/SESSION_RECOVERY_2026-09-10.md)与[唯一交付账本](../../../v2/APP_SCOPE_AND_UI_DELIVERY_GATE.md)。
...
### 可现代化增强的实现边界（不等于旧业务故障）

用户确认旧数据库和逻辑没有问题。以下项目用于在腾讯 V2、原生 App 和跨服务架构下保持或增强正确性、安全性与可维护性，不把旧系统描述成已经发生业务错误。

1. 旧实名方法按当时 V1 的 `Result == Success` 推进。迁移 V2 时应严格按 V2 的业务结果和固定阈值契约建测试，不能机械复用旧字段判断；这是版本适配要求，不认定旧 V1 逻辑错误。
...
5. 旧 `K_Register_SaveImage`、`K_Register_SetStyles` 和新用户注册分支已有数据库事务、异常处理与回滚，不能描述成旧注册失败会留下半套资产。Java 对两图的腾讯调用、对象 promote、分次保存和 token 清理跨越数据库事务边界；新版需要在保留旧数据库原子保障的基础上，增加申请版本、请求去重、可恢复任务与补偿，刷新状态不得重新收费。
...
3. **模块登录 UI 已获准独立接入**：2026-09-10 用户明确要求正式接短信验证登录并使用新会员表；当前手机号页通过 `AuthRepository` 接入，不再直接依赖页面内 Mock。
4. **短信登录联调**：App 使用 ECDH-P256、HKDF-SHA256、AES-256-GCM、HMAC-SHA256 调用 K107/K101/K102，设备实例号和会话密钥材料保存在系统安全存储；后端 K102 继续通过事务写入 `kingclubMember`。测试入口为 `api.sh-kingclub.cn`，IDC 为测试服务器。
5. **生产另审**：敏感材料处理、权限/日志、计费防重、规则版本、迁移及回滚测试完成后单独批准，不沿用 Mock 固定成功。

当前验证：Flutter analyze 无问题；真实短信和登录仍以 IDC 真机联调结果为最终验收。照片实名、两图上传与颜值评分仍保持下一阶段，不因短信登录接通而声明已完成。
