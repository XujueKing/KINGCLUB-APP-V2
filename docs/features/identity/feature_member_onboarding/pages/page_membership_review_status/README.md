...
> 2026-09-10 新要求：低分时显示“颜值分数不够，等待审核”，允许刷新及重新上传；审核通过后刷新可进入。UI/Mock 已改为读取共享申请快照，测试场景只更新模拟服务端状态，刷新后页面才变化；点击进入会再次复核，重传会撤销旧通过。见[最新核查/验收契约](../../2026-09-10-tencent-adapter-review.md)。当前仍待模块独立 UI 验收。
...
# 2026-09-11 真实注册状态页调整（最新）

用户要求重组页面、显示真实分数、使用尊重会员的描述并增加营销协助说明。读取 K303 已保存的 result.score（三张照片平均分），不调用评分或重新实名。展示“形象参考评分”，说明自动评分用于本次申请参考，不代表个人价值；不展示贬义评级或承诺一定通过。营销协助文案用于跟进审核、确认补充资料，真实联系方式待提供，不制造虚假客服入口。顶部沿用全局 13 号淡金标题，正文可滚动、底部操作固定。
