...
- **已确认事实**：全量验证通过 33 个测试文件、122 项测试、类型检查、构建、migration dry-run 和部署静态检查；空白 MySQL 8.4 已从 migration 001 完整执行到 021。
...
- **当前状态**：本地正常主链验收已完成；下一步补并发首次开户、权威不可用、绑定冲突、限流边界和 WebSocket 客户端观测异常矩阵，再进入四个 Flutter 登录页面的独立文档设计。
