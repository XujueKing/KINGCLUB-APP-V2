...
- 手机号占位强制水平、垂直居中；NEXT 与“获取验证码”使用浓郁暖金棕按钮面和香槟金文字，避免灰淡感，启用、禁用状态均须与黑色背景形成可辨识层级。
- `SHANGHAI . ZHUZHOU` 使用 13dp 与 70% 香槟金不透明度，弱化为页脚信息。
- 左上返回使用全 App 标准 `KingBackButton`：48dp 触控区、10×20dp 旧版箭头；在安全区内左偏移 18.5dp、上偏移 28dp，对齐旧版 750rpx 布局。
