...
### 2026-09-15 bound UDP socket handoff
...
### 2026-09-15 nearby LAN discovery adapter and user direction

Added Android NSD registration/discovery for `_kingclub._udp.` and its Dart platform adapter. Start is explicit and advertises an already bound UDP port with a random per-run token, no member name or account ID. Results are untrusted address hints. Discovery resolves serially with a 32-service working set and stops after 60 seconds, activity background, cancellation or session change; generation guards discard stale callbacks. No chat UI starts discovery yet and no Bluetooth implementation is enabled.

This working-set limit, and the preceding 16-relay default, are not SUPERVM routing-table or user-address-storage limits. User direction is now SUPERVM-primary decentralized communication, CCSOP fallback, with automatic discovery/reconnection after necessary OS authorization. Reuse upstream L4 ConsumerOnly/RoutingNode/RelayNode/NatAssistNode roles; separate persistent address-index capacity from active relay candidates and live connections. Mainnet-authorized offline identity, local message persistence/synchronization, node resource policies and automatic routing remain pending. Do not interpret current adapters as a complete decentralized chat service.
...
### 2026-09-15 historical peer identity observations

Production device-directory reads now save the validated peer public-key identities in the existing device-bound secure-storage namespace, isolated by account, own device identity and peer account. A later empty directory replaces previous keys. Records include observation time, are generation guarded, and expose immutable lists. Storage failure does not turn a valid online directory response into chat failure. The existing backup exclusions cover this namespace.
...
### 2026-09-15 reliable encrypted LAN text transport
...
### 2026-09-15 phone Wi-Fi reaches actual SUPERVM NAT services

ADB recovered to device; three shell reads succeeded, phone Wi-Fi was 192.168.1.21 and USB state mtp,adb. One SDK ADB server was running; another adb executable was present on PATH but no conflicting server was observed. Do not infer cable disconnection or demand unplugging from a transient offline/unauthorized result. Use the explicit SDK executable without restarting a healthy server.

Extended test-supervm-lan-node.py with explicit --adb and --serial. It sends synthetic signed probes through Android toybox nc over the phone's Wi-Fi. Host-side native code generates the ephemeral identity and verifies the returned upstream signatures. The first exec-out invocation supplied no UDP bytes; a controlled UDP diagnostic isolated stdin forwarding, then shell -T transmitted and received correctly. Both actual SUPERVM observer and punch services subsequently returned signed endpoints at 192.168.1.21, and tampered acknowledgements were rejected. No application reinstall, real account access, firewall change, ADB restart or message send was needed.

Scope: physical phone-to-computer UDP reachability and SUPERVM signed response interoperability, with cryptography on the computer. This does not prove cryptography inside the Android app, simultaneous peer-to-peer hole punching, same-port handoff across these two separate nc invocations, public network relay or automatic chat route switching. Bootstrap network fetching and app transport integration remain pending.

### 2026-09-15 bootstrap network loading design

Next integration: read explicitly configured bootstrap sources over HTTPS (literal loopback HTTP allowed for local integration only), with at most eight sources, bounded body and per-source deadline. Redirects are not followed; downloaded keys never become trust roots. Only a manifest passing the existing upstream signature threshold may produce relay candidates. Network failure, invalid signature or an empty usable directory tries the next configured source. Session change or explicit close must abort network work and discard delayed results. No source is enabled by default; sample domains are not used. This step supplies verified candidates, not completed data relay or automatic chat-route selection.

Implemented NovoRudpBootstrapLoader with the design above, a 12 KiB decoded response cap (leaving native request-envelope space) and fixed-format externally trusted Ed25519 identities. Actual loopback HttpServer plus real native DLL tests verify timeout fallback, redirect refusal, oversized-body rejection, forged-signature rejection, successful candidate extraction, all-invalid failure and pending-request cancellation. Two loader tests plus the existing native multisignature test passed; targeted analysis clean. No installed-app bootstrap source, live HTTPS bootstrap host or chat routing switch is enabled by this change.

### 2026-09-15 direct handshake rejection releases the attempt immediately

Found that native completion always consumes/closes the initiator handle, including rejected responses, but the nearby connector previously caught the rejection and retried the now-unusable offer until its eight-second timeout. It now closes the attempt and returns the validation error immediately. Malformed unrelated datagrams remain ignored; successful handshake and lost-answer recovery retain existing behavior. This prepares prompt route fallback but does not itself implement a routing policy.

Validation: an actual UDP peer generated a real native signed response, then altered its signature. The connector rejected within the two-second test bound, accepted no channel and could not reopen the closed attempt. Existing actual UDP lost-answer recovery and durable text/receipt-loss tests also passed: three tests total, targeted analysis clean. No APK installation or two-phone acceptance claimed.

### 2026-09-15 actual upstream WSS relay interoperability

Built the pinned upstream novovm-product-relay binary offline/locked in 7.99 seconds and started a two-hour LAN test daemon at wss://192.168.1.5:45172/novovm. scripts/start-supervm-relay-test.ps1 verifies/reuses its listener, uses the existing restricted test-identity directory, and generates a two-day local TLS certificate without installing a trust root into Windows or Android. Runtime identity/key/certificate/config remain outside Git. Only explicitly configured test clients trust that certificate.

The opt-in supervm_live_relay_test.dart connects two fresh synthetic native identities over actual WSS to the running upstream daemon, verifies signed relay authentication, relays the peer offer and answer, then sends an actual encrypted NovoRUDP frame through the relay. Receiver verifies source/target, decrypts the payload and rejects replay. One live integration test passed; analysis clean. Daemon report recorded registered_session_total=2, disconnected_session_total=2, forwarded_frame_total=3 and no protocol rejections; active connections/sessions and queues returned to zero. This is actual computer-client-to-upstream-relay interoperability, not two-phone media/chat acceptance or automatic app transport integration. No CCSOP or real account data was used.

### Relay client integration design

Add a session-owned App WSS client over the verified upstream wire format. Require a trusted relay identity and WSS endpoint, validate the native signed authentication before sending peer signalling or envelopes, keep the relay lease alive via heartbeat, and close on session changes, timeout or protocol errors. Caller must subscribe before connecting and independently authenticate peer signalling/decrypt envelopes; relay delivery is not a durable recipient receipt. Preserve approved UI and do not select this transport automatically yet.

Implemented NovoRudpRelayConnection with WSS-only endpoints, native signed relay authentication, explicit receiver subscription, peer signalling and opaque-envelope send methods, account-generation ownership, eight-second authentication deadline, heartbeat request/ack and close cleanup. Relay events are not treated as recipient persistence receipts. Replaced the live test's handwritten WSS registration with this App client; the actual upstream relay again delivered peer handshake and encrypted data, and both clients received heartbeat acknowledgements. Live test and targeted analysis passed. App routing still does not select the relay automatically.

User connected a second authorized Android device and explicitly assigned original=PCLM50 A and new=PKL110 B. Both are on the computer's LAN. All subsequent ADB operations must specify serial; do not use an unqualified install/start with two devices. B does not yet have the current preview package; A's data must remain intact.
