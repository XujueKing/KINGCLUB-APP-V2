...
Android13+通知权限：声明POST_NOTIFICATIONS，仅在首次前台服务成功启动且Activity仍前台时请求一次。拒绝/关闭弹窗不终止通话，不反复请求；此前已经授权则不弹。先启动服务再请求通知，避免权限弹窗令Activity暂停影响麦克风服务启动。依据Android官方notification-permission说明，通知权限不是启动FGS的前提；拒绝时系统任务管理器仍可显示运行服务，通知栏不保证显示。此增量尚未安装B进行Android15权限验收，不属于系统来电推送实现。
