...
当前手机已安装 a058292 工作区对应 Profile/preview ARM64 包，包含头像失败重试、视频校验和分块加密后台计算、通话失败清理修复及表情云同步客户端。表情云同步在线接口尚未发布，本机表情仍本地保留。构建时保留了用户尚未提交的 onboarding 三文件改动，因此不是纯 Git 提交快照。安装成功与启动正常不代表实际聊天全流程验收通过。
