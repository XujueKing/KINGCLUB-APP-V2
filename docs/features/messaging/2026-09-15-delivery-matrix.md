...
修复首条格式错误的确认阻断后续会话补交：异常意图保留并冷却，正常单聊/群聊继续处理。29 项相关测试、两文件静态分析通过，尚未安装。详见 `2026-09-16-read-retry-isolation.md`。
...
单聊搜索/引用进入的历史上下文使用服务端通话元数据显示对应语音或视频入口，并复用当前会话回拨链路。原聊天页面的回拨原已实现，本次补齐历史入口。11 项测试、4 文件静态分析通过；尚未安装及真实拨号验收。详见 `2026-09-16-history-callback.md`。

## 2026-09-16 已读回执确认

单聊/群聊即时及后台补交均校验服务端 readSequence，群回执同时匹配 groupId；异常结果不删除待补交记录、不发布成功通知。31 项相关测试通过，尚未安装手机。此前群聊已读持久队列已接入，本次补强确认语义，不将受控测试计作真实断网重启验收。详见 `2026-09-16-read-ack-validation.md`。
