...
A 向 B 断网发送测试文字后显示等待网络，保持断网强制结束并冷启动后仍保留；恢复网络后无需手动重试自动提交，服务端完整文字搜索返回一条记录。A 网络恢复原状态。B 实际接收显示、服务端成功但回执丢失及群离线发送仍待验收。详见 `2026-09-16-offline-send-acceptance.md`。
