...
2026-09-18 latest source batch **ba6ee30**: ARM64 Profile preview built (155.7s), **1052 passed / 10 skipped / 0 failed across 180 selected files**. Includes durable group access invalidation, offline queue/readmission checks, direct send permission updates and forwarded-video background verification. **Not installed on A/B**; replaces the previous artifact at the shared path. [Artifact and exact scope](../../audits/2026-09-18-chat-offline-batch.md).
...
2026-09-18 earlier client batch: **72902aa ARM64 Profile preview built**, 204.0s Gradle, native ELF validation passed; **not installed**. Consolidated selection now includes foreground/direct/group call tests: **1026 passed, 10 skipped, 0 failed across 179 files**. First run exposed a fixed-time filesystem test race; its bounded durable-completion fix and full rerun are recorded in [batch artifact and regression evidence](../../audits/2026-09-18-chat-batch-build.md). The new APK replaces 758ab7d at the shared artifact path. It includes fba1860, 4511389, 9e0e9fb, 9816c14, 48cef77 and group call client 72902aa; no uncommitted onboarding changes.

Group call records: server cd80ea9 returns authoritative group-scoped metadata linked to migration 123 records. Client 72902aa persists it locally, renders existing call-row style in conversation/context, and opens member selection on tap without automatically calling. No duration is fabricated. Service e297f0e passed the complete isolated SQL/encrypted HTTP/WebSocket/media-resource integration, including viewer-only hide and recall metadata removal. Migration 122 was added to the isolated test database for the current group-management module. **Production deployment and real group-phone DTLS/SRTP/audio/video acceptance remain pending.** The built client cannot make an undeployed server field available.
...
本节优先于下方按开发过程记录的“未安装”“待复测”等历史状态；下方记录保留用于追溯。A 最近安装的预览提交为 `f76deb7`；下述整批修复尚未安装，B 未随这些节点更新。以下“已验证”只表示表内写明的范围，不表示整项功能全场景交付。

| 能力 | 已有证据 | 尚未完成 |
|---|---|---|
| 普通单聊、双人群文字 | 真实接口、双机基本收发；A 离线待发冷重启后自动补交，服务端搜索仅一条 | B 最新包恢复接收、回执丢失及多端全场景 |
| 好友资料、关系分组与群成员搜索 | A 群成员跳转真实资料；分组保存、离线保留及删除；新版成员搜索、无结果及清除恢复实机通过 | 多端同时编辑和全部权限变化 |
| 离线历史搜索及上下文 | A 单聊/群聊在系统无默认网络时搜索已存文本并打开上下文 | 大规模有效查询性能；手机端文件名搜索及离线引用跳转 |
| 上下文图片、视频、语音 | A 离线全屏查看既有图片、8 秒视频播放到末尾；3 秒群语音有解码和音轨输出证据 | B 更新、全部媒体格式和主观音质；不能据此证明所有发送源缓存分支 |
| 文件持久化与删除 | A 12 MiB 项目测试文件离线冷启动读取、导出哈希一致；删除消息后本地源分块移除，导出副本保留 | B 大文件接收、转发及实机中断续传 |
| 清空/删除对应本地内容 | 本轮 18 项清理、草稿、删除事件及附近消息副本回归通过；已实现历史、引用和媒体删除防迟到回写 | 待发队列仍保留；跨存储的并发附件引用/延后回收不能宣称全量解决 |
| 群邀请及管理 | A 新版邀请空列表与前后台恢复通过；成员上限 200 的源码核查及列表懒加载测试通过 | 非空邀请接受/拒绝、审核/退群/转让真机验收；不是 200 人实机压测 |
| 语音及视频通话 | 用户已确认 A/B 双向声音、视频正常；靠近双外放才啸叫 | 跨公网、复杂声学场景及群通话实机验收 |
| 语音转文字 | 服务端 119 已发布；A 短语音识别已有实际验证 | 长录音、噪声、并发容量与准确率评估 |
| 表情云、媒体分类搜索、群禁言 | 存在客户端及隔离后端验证 | 对应 120/121/122 未部署，不能作为线上交付 |
| 加密和 SuperVM 自适应 | 已有 TLS、本地加密、测试直连/中继及故障回退证据 | 完整端到端加密、公网主网、手机节点及跨网络自动切换未完成 |
| 金币、红包、礼物、卡券、系统推送 | 仍按各自待办推进 | 完整业务交易未交付；系统推送按用户安排后置 |

主要证据：
- [离线搜索及上下文媒体实测](2026-09-17-local-history-search.md)
- [群邀请生命周期与 A 成员搜索实测](2026-09-17-group-invitation-lifecycle.md)
- [12 MiB 文件离线、导出及删除](../../audits/2026-09-17-large-file-offline.md)
- [待发消息附件引用与现有限制](../../audits/2026-09-17-pending-media-reference-cleanup.md)

整体目标仍未完成；未接通入口、受控测试和安装成功均不替代真实验收。
...
## 图片刷新范围修正
图片组件不再因无关群已读/群变化或明确属于其他会话的事件清空重载。单聊忽略群事件，群图片携带当前groupId；当前群已读仅后台重验授权，保留已显示图片直至响应，拒绝时立即清除。成员/关系变化仍先清旧授权，迟到响应仍按generation丢弃。适用于聊天缩略图、全屏及历史上下文。
图片组件8项测试、相关4文件静态分析及diff检查通过。覆盖单聊/群无关事件不触发请求，当前群已读重验期间图片连续显示、拒绝后清除，以及旧有账号切换/迟到授权保护。仅组件验证，本节点未安装手机。

## 视频通知范围
视频缩略图、全屏播放、历史上下文接入与图片相同的会话范围过滤。明确其他群/单聊的事件不触发下载或重建播放器，当前群已读仍保留播放位置后台核验；无scope事件保守核验，账号变化及真实撤权仍清除。
图像/视频组件15项测试、6文件静态分析通过。播放器测试验证无关事件不增加授权/下载/播放次数，不销毁播放器且保持7秒位置；既有当前群撤权、迟到响应、HEVC回退用例继续通过。使用受控播放器，并非真机播放验收；暂未安装。

## 2026-09-16 A 端媒体刷新修复安装

6cb9a4d 后工作区 Profile/preview ARM64 构建成功（Gradle 66.2 秒，150.7 MB），启用真实 API、NovoRUDP 设备绑定、文件和 LAN 开关。APK SHA256：32B78DDB61B5E5A17BED61AE179FDCC7CCB04580530DC29AE2BF42535DF28C6A。包含既有未提交 onboarding 三文件，未将其提交到本节点。

仅 A（462606d8）覆盖安装 Success；启动助手第一次验证连续前台成功。实际进入消息及 B 好友会话，既有语音通话 00:27、视频通话 00:14、表情及 Shanghai 位置消息仍展示，未见登录变化或关系结束提示。未发送新消息、未操作 B。该检查只证明安装和页面恢复；未验证本次图片/视频实际播放、通知期间连续播放及双机 LAN 路径。

## 2026-09-16 LAN 多地址寻址

修正只选择第一个同 /24 候选地址的问题：受认证成员通道每次最多提供四个私有 IPv4 地址，去重后逐个发送加密探测。仅成功收到加密 pong 的地址成为数据路径；未确认候选不能投递应用数据。已选路径保持稳定，失效后重新探测候选，未确认期间仍由原有链路回退中继。未增加全网段扫描、未放宽身份验证或端点来源限制。

真实 UDP / 原生加密测试把不可用的子网网络地址放在可用接口之前，验证仍建立直连；继续验证关闭对端 socket 后失效、重新绑定端口后恢复并交付内容。真实 SUPERVM TLS 中继和 LAN 文件测试各传 262161 字节、丢弃最终回执后仍校验一致；共三项通过，静态分析通过。此证据为当前电脑上的测试，不是双手机多网卡验收。本节点尚未安装 A/B。

## 2026-09-16 直连中断后同笔文件回退

新增真实 LAN 中断用例：两个真实原生加密通道和 WSS 中继；已接收超过 16384 字节时关闭接收方真实 UDP socket，保留身份通道与中继。验证发送端无需重新创建发送任务即自动回退，接收文件 262161 字节和摘要一致，首次最终回执丢失后仍完成。与纯中继、纯 LAN、多候选及端口恢复合计四项测试通过；静态分析通过。注入的仅是打开真实 socket 的工厂，生产默认仍为 NovoRudpLanRoute.open。

测试最初失败，发现中继空闲投递预算只有每轮四帧，补包积压。已在 SUPERVM 579008d1 按原有客户端内存预算派生每轮11帧数据+4帧控制上限，12项中继测试通过，本机重建并启动PID19508。改后中断用例14.298秒完成；这是小文件/本机验收，不是双手机、弱网或大文件完成证明。未保留无效的客户端固定延时实验，手机包无须为服务侧修复重装。

APP节点推送正常；SUPERVM origin仍为novovm/supervm，XujueKing推送返回403，该提交仅本地，不能称已推送。

## 2026-09-16 App 下载器切换超时验证

在前述真实中继/LAN夹具中增加 NovoRudpFileDownload 路径，保留默认8秒无新增片段超时及5分钟总上限。接收超过16384字节后关闭真实UDP socket，同一发送任务回退WSS并完成262161字节下载，接收字节计数及文件内容一致，14.218秒完成。该用例通过，静态分析通过；未更改生产超时值。此路径未丢弃最终回执，回执丢失由已有独立用例覆盖。授权回调为测试允许，不证明真实会员撤权/手机网络切换。

## 2026-09-16 A 端群成员搜索及资料跳转

在 A 已安装真实接口包进入 KINGCLUB-AB-0915-OK 测试群及群聊资料，显示两名真实成员，免打扰/置顶均关闭。搜索输入 B 账号前缀 UMAD 后仅保留 B；点击清除恢复两名成员。点击 B 成员行进入资料页，会员号 KM0000000028，互相关注、私信和作品/动态/相册入口展示；当前作品为空。此为真实手机页面及接口数据检查，不是组件 Mock。

未改名、未修改备注/设置、未邀请或移除成员，未操作 B。A 页面未显示群主管理项；本次没有执行服务端越权验证，不能据此宣称群权限全量验收。截图/层级私存 build/group-*-ui.xml，不提交会员页面原始数据。

## 2026-09-16 A 端群视频真实播放

从真实测试群打开既有8秒视频消息，全屏显示测试色条/时间码视频。首次观察已到结尾；点击播放后可见0秒画面及暂停按钮，随后约7.93秒末帧和完成进度重新出现，证明已安装原生播放器实际解码并重播。本机私存截图 build/group-video-playing.png、group-video-replay.png、group-video-replay-end.png。检查后返回聊天。

这只覆盖该条既有群视频在 A 的打开、播放及重播，不证明音频听感、任意格式、跨会话通知播放连续性或 B 端表现。未发送新媒体，未操作 B，声音未人工确认。

## 2026-09-16 语音转文字独立上线

后端6f91a3f已在原测试API最小发布119：674本人录音、675单聊/群聊语音识别，以及对应AAC转PCM和识别客户端。未合入表情云/群通话等其它待发布能力。与线上118镜像同基底的隔离库+真实加密HTTP+真实中文模型，完成上传、本人识别、单/群消息识别和外人/隐藏/撤回拒绝验证；测试会话已撤销。全量98文件420测试、静态及运行时部署检查通过。

线上健康、目录、模型可达及运行配置比对通过；原环境除新增识别URL外完全相同，端口/挂载/网络/重启策略保持，旧容器保留回滚。识别容器无公网映射，已设置自动重启。此段替代本文件早先“ASR未发布”状态。手机已装客户端入口，但本次尚未完成手机端识别验收；自然噪声、长录音及并发容量未验收。普通识别不自动发送消息。
