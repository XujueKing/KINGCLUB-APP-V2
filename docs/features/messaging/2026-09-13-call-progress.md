...
界面状态后增加本机mm:ss时长，首次原生RTCPeerConnection connected事件启动Stopwatch，连接恢复不清零，包含接通后的短暂恢复等待；结束/关闭立即停止，不等服务器结束回执。单调时钟不依赖系统墙钟。每秒仅刷新当前非结束通话页面，销毁取消定时器。此为本机通话时长，未伪造服务端历史通话时间。

9项控制器/页面测试通过；增加受控时钟断言：连接前无时长，首次及再次连接只启动一次、65秒保持、网络恢复超时立即冻结。定向analyze通过。未安装，不代表双机真实音视频接通验收。


### 2026-09-15 公网TURN再核验

电脑探测UDP/TCP超时、TLS握手EOF；已连接Android通过toybox nc向相同TURN主机发送无凭证STUN Allocate，UDP/TCP未收到有效401挑战。手机只写入28字节合成请求，探测文件已清理；未录音或发送会员消息。服务器内网UDP/TCP返回401挑战，内网IP进行TLS校验失败不能当作域名TLS失败。主机3478UDP/TCP、5349TCP监听，UFW已有允许规则且TURN使用host网络，未重复改防火墙。

更正此前入向1、出向0的计数：旧awk无条件累计非响应行，tcpdump空行也被计入，不能作为收到请求的证据。重新仅匹配真实IP报文，手机直接向服务器公网IPv4发送同一28字节请求，14秒eth0观察为UDP_REQUESTS=0，手机响应文件0字节；另行确认tcpdump能正常启动且无内核丢包。SSH目标与云主机元数据公开地址一致。当前证据指向请求尚未到达主机抓包点，仍不能据此唯一确定是运营商路径、云安全组或其他上游策略；未修改主机防火墙。不将公网分配/媒体中继标记可用，下一步需核对云网络入站规则和端到端路径，再做鉴权allocation及媒体回传验证。


### 2026-09-15 通话计时包已安装

基于f287a7c构建preview profile ARM64真实接口包，Gradle 62.9秒、162.2MB。SHA256 `e01d2416356d60396f528f17038673cba0fc00d8c087d329862801f73595c0f3`。连接测试手机adb install -r返回Success，保留数据；显式启动后进程存活，定向AndroidRuntime日志未见异常。包含021f221计时改动；工作区原有onboarding三文件随包构建，本节点未修改或提交这些文件。未发起真实联系人通话，计时显示和双机媒体仍待真机验收。
