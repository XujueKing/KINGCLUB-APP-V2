...
A 在同一测试群发起视频通话，首次摄像头权限选择使用时允许，随后显示本人“已加入”。摄像头关闭后按钮变为“开启摄像头”，再次开启后恢复“关闭摄像头”，无失败提示。退桌面时前台通话服务仍有效，返回后本人仍为“已加入”。B 未接听，邀请正常变为已超时。

主动返回退出后，服务端最新记录 mediaKind=video、version=7、ended=1，群通话占用为 0。这证明该 profile 包的本机音视频发布、摄像头暂停恢复及短暂后台返回路径可用，不能代替对端画面/声音、长时间锁屏和多成员验收。
