...
聊天页主动拨号此前未设置 App 的来电展示标记。现在单聊/群聊的主动拨号与前台来电共享 CallPresentationLease，拨号异步准备前获取，流程结束 finally 释放。旧流程重复释放不能解除新流程的占用。来电遇到占用返回未展示，交由现有 inbox 后续读取，未自动接听/拒绝其他来电。

静态分析通过；展示占用、聊天页单/视频拨号入口、单人/群前台 inbox 共 9 项测试通过。此修改未安装到手机；不代表真实并发来电或原生采集互斥已经双机验证。系统后台通话仍未实现。

## 2026-09-15 群通话静音成员状态

页面此前只用可展示的非暂停流判断成员是否连接；音视频都暂停时错误显示“连接中”。现在独立记录已订阅成员的音频暂停状态，保留真实服务端暂停值；关摄像头继续使用头像，静音显示“已静音”，并在登录变化、结束和退出时清理状态。不把是否存在画面当作成员连接状态。

页面静态分析通过，既有群成员选择/拨号重试页面回归 1 项通过。该测试未覆盖真实远端静音画面，双机视觉与声音验收仍待完成。本改动尚未安装。

## 2026-09-15 拒绝群邀请的幂等重试

前台邀请拒绝改由同一账号仓库保留本次 requestId 和原 expectedVersion。网络失败后的用户重试重放原请求，不因读到新版本而新建操作；仅明确版本冲突才清除本次身份。成功清除记录，新 callId 替换旧记录，内存最多保存一个拒绝尝试。不自动接受邀请或开启采集。

相关静态分析通过，群调用仓库、前台邀请、群通话控制器共 17 项通过，覆盖丢失响应与明确冲突两类请求参数。尚未安装，真实弱网双机拒绝流程未验收。

## 2026-09-19 网络切换时中继凭据读取重试

发现：已打开群通话发生 disconnected/failed 后，刷新中继凭据的一次 NETWORK_ERROR 会直接关闭媒体，早于已有 ICE 恢复期限。

修正：仅在已建立媒体的刷新路径，对只读 readRelay 最多尝试三次，两次重试间隔各一秒；初次开启采集、凭据格式/有效期错误、会话或权限错误不重试。取得有效凭据后才执行原有两路 ICE 更新；不自动重放 ICE、发布或订阅写操作。原 25 秒连接恢复期限和消息源读取期限不变。挂断取消待重试计时器并唤醒等待，后续检查禁止再次读取或更新原生连接。

验证：静态检查无问题；native_group_call_media 与 group_call_session 共 30 项测试通过，覆盖首次网络错误后恢复、三次失败耗尽、权限错误立即结束、重试等待中挂断不再请求，以及既有收发恢复/采集释放流程。日志 build-group-relay-retry-analyze.log、build-group-relay-retry-tests.log。

客户端改动尚未打包安装。测试中的原生设备/仓库替身不代表真实网络切换或公网群通话验收；服务端发布仍待确认。
