# 单聊详情页

- Scope ID：`KC-P-025`
- 文档状态：`Approved for Development`
- 所属功能：[稳定单聊](../../README.md)
- 路由：`DirectChatDetailsRoute`，`/messages/chat/details`，`$extra: ConversationRef`
- 设计版本：`Direct Chat Wireframe v1 / Details`
- 最后更新：2026-08-25
- UI 状态：`UI Mock Implemented（2026-08-27）`

## 用户任务与线框

查看对方、调整当前单聊设置、搜索聊天记录，或为自己清空记录。

```text
[返回]              单聊详情

[头像] 好友备注 / 公开昵称                    >

消息免打扰                              [开关]
置顶聊天                                [开关]
搜索聊天记录                                >
关系权限                                    >

[为我清空聊天记录]
```

- 不含群成员、加减成员、群二维码、群公告、群管理、聊天背景或退出群聊。
- 搜索在当前页面进入内嵌 search mode，不新增逻辑页面；只搜索本人仍可见的文本投影。
- clearForMe 对当前用户隐藏服务端游标之前的历史，对方不受影响；不可恢复，需二次确认。

## 旧版 UI 复刻

UI/Mock 实现以[旧版聊天详情 UI 复刻规范](legacy_ui_replication.md)为准，保留旧版分组设置列表和开关样式，并删除不属于本期单聊的群管理与聊天背景入口。

状态见 [states.md](states.md)，交互见 [interactions.md](interactions.md)，验收见 [acceptance.md](acceptance.md)。

## 2026-09-14 真实头像补齐

聊天详情顶部保留52dp头像布局，使用当前仓库授权的好友资料接口，点击进入现有真实 PublicMemberPage。请求仅在页面初始化时创建，失败显示中性占位，不增加整页加载。真实会话进入详情不再将语音、图片等非文本消息强转为字符串。

验证：6项定向测试通过（真实详情头像、授权失败占位、会话续期），3个修改源文件 analyze 无问题。此增量尚未打包安装，手机上仍为上一轮已安装版本。
