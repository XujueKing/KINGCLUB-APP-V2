...
单聊/群聊的普通文字可转发为新的普通文字消息。选择真实好友（含备注搜索与授权头像），明确确认内容后持久入队并打开目标单聊，复用现有离线重试及服务端私信/拉黑校验。队列失败留在选择页并复用本次ID；登录变化清空选择。仅本批文字转发，不将附件占位文字作为媒体转发，群目标和媒体转发仍待接入。

验证：forward-text-tests.log 两项页面测试通过，覆盖确认前不入队、保存失败重试同ID、离线进入真实目标会话、确认期间账号变化关闭内容且不发送。两个源文件 analyze 无问题。尚未手机安装与双机收发验收。
