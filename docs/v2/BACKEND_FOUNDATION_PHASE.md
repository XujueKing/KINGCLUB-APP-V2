...
ccsop-service + kingclub business line
  ...
  `-- MySQL 8.4 KingClub V2 database
        ...
        |-- kingclub domain tables
        `-- legacy ID mapping + migration audit
```

- **当前建议**：为 KingClub V2 建立独立 MySQL 8.4 数据库或独立部署实例，先执行服务端底座 migration，再添加 KingClub 业务 migration。
...
- **当前建议**：现阶段业务数据库仍独立，但从第一天引入全局 `personId`、App 级 `appMembership` 和旧 ID 映射，避免未来只能依靠手机号硬合并。
- **当前建议**：统一身份只共享自然人和登录身份；昵称、头像、审核状态、会员、钱包、金币、订单和业务权限默认按 App 隔离。
- **当前建议**：未来再把身份表抽成独立 `city_identity` 数据库或身份服务，物业和 KingClub 逐步切换，不在第一期同时迁移全部物业业务。
...
5. [同城统一身份](../features/identity/feature_unified_city_identity/README.md)

四个工作包都通过评审和验收前，不开始 Flutter 登录页或业务页面开发。

## 6. 待用户决策

1. 统一身份数据库是第一期立即独立建设，还是先在 KingClub 独立库使用一致模型并预留迁移能力。
2. Flutter 首期登录入口是短信验证码、原生微信授权，还是两者并行；底层会话协议已确认复用成熟方案。
3. 同一 KingClub 账号在新设备登录后，旧设备是立即强制退出还是允许短暂提示确认窗口。
