...
四个 Flutter 登录页面已经归档到 `feature_login_session/pages/`，并全部达到 `Approved for Development`。ADR-0001 已批准，本机已升级到 Flutter `3.47.1 stable / Dart 3.13.1`。app_bootstrap 已批准；navigation 已按当前五个目标语义精简，订单、聊天、申诉、通用 restricted、业务深链和推送跳转不进入当前实现，模块仍为 `In Review`、待用户批准。
...
2. 48 个页面均已建立独立文档目录：四个登录页已批准；App Shell/信息架构、navigation 和 Design System v1 已形成完整评审包并处于 `In Review`。全局提案为“首页、消息、发现、我的”四主目的地 + 中央扫码动作，私人储物柜从我的进入，首发仅深色主题。
3. 用户确认全局评审包后更新准入状态；随后按 identity → home/profile → social/messaging → club/commerce → wallet/content 顺序，逐项完成并批准产品、交互、UI 状态、Mock 场景和验收文档。networking、session/persistence、observability 只定义 App 端 port、Fake 和未来 adapter 契约。
