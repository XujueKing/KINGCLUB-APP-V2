...
四个 Flutter 登录页面已经归档到 `feature_login_session/pages/`。启动鉴权页和手机号登录页已批准；验证码页和协议确认页仍为 `In Review`。下一次继续：

1. 评审验证码页的自动提交、粘贴/系统填充、错误次数、重发和安全预览策略。
2. 确认协议版本变化返回 `AUTH_CONSENT_VERSION_INVALID` 后保留有效 challenge，并重新读取 K107 的流程。
