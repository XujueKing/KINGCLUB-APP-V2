...
2. 48 个页面均已建立独立文档目录：现有四个登录页已批准，其余 44 页及其功能目录均为 `Draft`。下一步先评审全局 App Shell/信息架构和精简后的 navigation，再完成 design_system 文档。
3. 随后按 identity → home/profile → social/messaging → club/commerce → wallet/content 顺序，逐项完成并批准产品、交互、UI 状态、Mock 场景和验收文档；networking、session/persistence、observability 只定义 App 端 port、Fake 和未来 adapter 契约。
