# 2026-09-12 五服务项柔和色与留白

- 390×844中文截图已复核：五SVG及文字#B7ADA0，26dp画布、12dp字体，服务行下方额外10dp。头部、心情和底栏未改。
- analyze及8项已有布局/滚动/字体/截图测试通过。

# 2026-09-12 菜单宽度与心情间距

- 已查看390×844中文展开截图：服务行扩宽至718rpx，图文及上下留白收紧，创作者中心单行。金额下新增心情行，空态可点编辑，保存签名后即时回显。
- 11项测试通过：320宽1.2倍字号服务单行/宽度、心情上下位置/编辑联动、200%大字、原生吸顶、封面持久化及中文截图；analyze通过。

# 2026-09-12 顶部位置与五SVG复核

- 390×844中文展开/半淡出/吸顶截图已核对：左二维码+经验值换昵称，右设置不移动、不淡出；五项SVG小图形同基线、12dp文字，全部本地加载。原始1024坐标和path保留，统一主题色。
- 7项顶部/淡出/五SVG/200%字号及截图检查通过；48项App冒烟和原生滚动回归通过，analyze无问题。首次碰碰功能仅入口，业务未接。

# 2026-09-12 我的吸顶渐隐复核

- 390×844中文展开/距吸顶80dp/完全吸顶截图已复核，build/profile-qa/approaching-pinned.png为半透明中间帧；消除工具栏后方资料文字透出。二维码与设置去背景及水波纹。
- 7项定向测试通过，含100%→50%→0%→50%反向连续过渡、隐去EXP不可点击但二维码可用、工具栏透明、大字及原生菜单几何。未声称真机FPS或与抖音逐像素相同。

# 2026-09-12 个人主页资产与服务入口补充

- 已对照新图采用图上字下的四列服务行，保持黑金色；订单、预约、创作、专属客服使用旧gouwudai/menu_list/edit/tabBar_chat图片。余额胶囊恢复旧moneyStyle高度、圆角、颜色，性别改用man5原图。
- 390×844中文截图已检查服务行、资产密度及页签位置；320/430宽200%字及四项等宽/位于页签上方测试通过。专属客服为最终文字名称，帮助面板未接人工会话。

# 2026-09-12 我的栏目原生滚动还原

- 参考当前旧仓库index.wxml/index.wxss/index.js，与历史已撤销远端UI隔离。封面400rpx、头像180rpx从260rpx开始、面板左右40rpx、横排统计与自适应编辑动作；当前旧版默认作品。
- 390×844、状态区44/底部34、微软雅黑字体的截图保存在build/profile-qa/expanded.png及pinned.png；截图仅是预览数据，不代表真实会员信息。检查封面/头像、资产同一行、标签换行、菜单吸顶及底栏。初版截图未等待素材，已补预加载；资产字号12dp保留34dp最小高度，选中箭头复用原select.png。
- 320×568和430×932的200%字号测试无溢出；真实手感仍需手机验收，不宣称达到特定FPS或完全像素一致。主底栏视觉文件未修改。
...
- 偏好：旧截图含旧进度条/选项/布局，新实现对应已确认的五步骤小标题、无进度条、NEXT及固定底部。修正测试视口为393×852并等待返回箭头素材加载，复核间距、选中及未选中状态后更新四张基线。
- 聊天：差异为素材未加载，预加载后原基线通过，未更新聊天截图。
- 本轮未改变已确认页面视觉。全量389项测试通过，2项需字体配置的独立截图测试跳过。测试默认Ahem字形仅适用于像素回归，不代表实机中文字形验收。


# 2026-09-12 撤回远端合入

用户已撤销本次全部远端更新，包括我的栏目；页面恢复合并前本机基线。上方回归结果为撤回前历史，本次验证见合并审计报告末尾。

# 2026-09-12 储物真机反馈修正

- 用户反馈：瓶顶距标题太近、满瓶液面顶到瓶口、过期物品QR图标点击无响应。
- 改动：展示区顶部48rpx内部留白；满瓶视觉液面90%高度并继续波动；QR入口不禁用，过期详情给出原因而不签码。
- 渲染证据：build/storage-qa/wine-back.png，390×844、满瓶100%测试数据；已查看标题与瓶口间距、瓶颈留白、网格和底栏位置。自动化7项通过；真实过期数据未续期。

---

# 2026-09-12 储物柜截图对照

- 来源：当前用户提供的旧版柜子、券、瓶背面、取件页截图与只读旧小程序素材。
- 本地渲染：390×844 dp，安全区44/34，build/storage-qa/wine-front.png、wine-back.png、coupon-front.png、coupon-back.png；真实Flutter组件、仅fixture数据。
- 已检查：原PNG完整加载，三列九格与数量、酒/物、金色轮廓、百分比、翻转切面。初次捕获未等券图片解码，已预缓存素材并重新捕获确认。
- 与参考的有意差别：无小程序右上胶囊；保留用户后来确认的90%茶色磨砂底栏；提取码保留短码而不显示内部仓位、员工、永久存酒编号。尚未完成真机有库存状态人工验收，不宣称99%像素一致。
- 布局测试覆盖360×640、393×852、430×932；短屏可滚动。
