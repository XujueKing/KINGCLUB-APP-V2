...
void main() {
  for (final mode in ['audio', 'video']) {
    testWidgets(
      '$mode dial entry suppresses repeats, retries the same request and discards a late result after leaving',
      (tester) async {
        ...
        Future<void> dial() async {
          ...
          await tester.pumpAndSettle();
          if (find.byKey(ValueKey('call-type-$mode')).evaluate().isNotEmpty) {
            await tester.tap(find.byKey(ValueKey('call-type-$mode')));
            await tester.pumpAndSettle();
          }
        }
        ...
        await tester.pumpAndSettle();
        expect(requests, isEmpty);
        // Close the attachment panel so the dial helper starts from idle.
        await tester.tap(find.byKey(const ValueKey('direct-chat-attachments')));
        await tester.pumpAndSettle();
        await dial();
